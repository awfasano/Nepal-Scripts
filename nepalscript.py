...
# Function to match legend variables with dataframe variables
def legend_match(legend_df, df_variables):
    match_status = legend_df.copy()
    match_status['Matched'] = match_status['Variable Name'].apply(lambda x: 'Yes' if x in df_variables else 'No')
    return match_status
...
legend_file_path = '/Users/awfasano/Downloads/legend.xlsx'

# Read legend variables from the legend file
...
for file_path in file_paths:
    try:
        ...
        for sheet_index, sheet_df in enumerate(sheets):
            ...
            all_variables.update(df['Variable Name'].unique())
...
# Match legend variables with dataframe variables
legend_match_df = legend_match(legend_df, all_variables)

# Write the legend match results to Excel
legend_match_df.to_excel('/Users/awfasano/Desktop/Nepal Scripts/inputs/legend_match.xlsx', index=False)

print("Legend match results written to legend_match.xlsx")
